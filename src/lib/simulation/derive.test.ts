...
describe("deriveSubScores (the teachable mappings)", () => {
  ...
  it("flags wrong-format for a text-only MockTube (video) post", () => {
    const d = deriveSubScores({ ...base, platform: "mocktube" }, ctx);
  ...
  it("an image on MockGram (image platform) scores top format", () => {
    const d = deriveSubScores(
      { ...base, platform: "mockgram", hasImage: true },
      ctx,
    );
  ...
  it("flags hashtag spam past 8 tags", () => {
    ...
    expect(deriveSubScores({ ...base, hashtags: tags }, ctx).gates.hashtagSpam).toBe(
      true,
    );
  });
  ...
  it("evening weekday timing beats 3am timing", () => {
    ...
    const small = deriveSubScores({ ...base, postingMinute: 3 * 60 }, ctx)
      .subScores.timing;
  ...
  it("freshness decays as the campaign clock advances", () => {
    ...
    const stale = deriveSubScores({ ...base, ageDays: 30 }, ctx).subScores
      .freshness;
