...
describe("performanceScore (ADR-0001 validated shape)", () => {
  it("is deterministic for identical inputs", () => {
    expect(performanceScore(STRONG).score).toBe(performanceScore(STRONG).score);
  });
  ...
  it("geometric mean punishes one broken factor harder than a weighted sum would", () => {
    ...
    expect(performanceScore(oneBroken).score).toBeLessThan(
      performanceScore(MEDIOCRE).score,
    );
  });
  ...
  describe("gate penalties (the teachable cliffs)", () => {
    ...
    it("stacks multiple gates multiplicatively", () => {
      const r = performanceScore(STRONG, { wrongFormat: true, hashtagSpam: true });
...
describe("projectReach (low floor, steep slope, within-platform)", () => {
  it("is deterministic given the same seed parts", () => {
    const a = projectReach(0.8, PLATFORM_BASELINE.twitter, ["post-1", "c-1"]);
    const b = projectReach(0.8, PLATFORM_BASELINE.twitter, ["post-1", "c-1"]);
    expect(a).toBe(b);
  });

  it("a near-zero score still earns a small floor of reach, not zero", () => {
    const reach = projectReach(0, PLATFORM_BASELINE.facebook, ["p", "c"]);
    expect(reach).toBeGreaterThan(0);
    expect(reach).toBeLessThan(REACH_FLOOR * 1.3 * PLATFORM_BASELINE.facebook);
  });

  it("higher scores earn more reach on the same platform", () => {
    const lo = projectReach(0.3, PLATFORM_BASELINE.instagram, ["p", "c"]);
    const hi = projectReach(0.9, PLATFORM_BASELINE.instagram, ["p", "c"]);
