 ...
 * Performance score math. Always visible to Students; the numeric score and
 * factor breakdown stay teacher-only (ADR-0001).
 *
 * Pure and deterministic — derived from the same content + factors the score
 * uses, so the qualitative story always matches the quantitative outcome.
 */
...
/** Build the hint chips for a post. "Bad" hints (what to fix) are prioritised. */
export function buildHintChips(
  ...
  // Keyword strategy (only meaningful once the Student has defined keywords)
